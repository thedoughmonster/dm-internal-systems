...
function buildInitialPrompt(selectedDirective, selectedTask, launchConfig, roleTransition, role, runbookPhase) {
  ...
  const architectDiscoveryChat = role === "architect" && runbookPhase === "architect-discovery";
  const lines = [
  ...
  if (autoExecuteExecutorTask) {
  ...
  } else if (architectDiscoveryChat) {
    ...
    lines.push("Do not use numbered/bulleted interrogations by default.");
    lines.push("Ask one natural clarifying question at a time and adapt to the operator's wording.");
    lines.push("Do not ask model/thinking gates during discovery-only conversation.");
    ...
    lines.push("When operator approves, transition to architect authoring for directive creation.");
  ...
  if (role === "architect" && !selectedDirective) {
    ...
    lines.push("Do not create directive artifacts in discovery.");
    lines.push("After approval, transition to architect authoring phase via runbook to create directive artifacts and continue structured task authoring.");
    lines.push("Just talk to me now, let's figure this out together.");
  }
  return lines.join("\n");
}
...
function writeStartupContext(root, args, bundleArgs, role, profileName, selectedDirective, selectedTask, launchConfig, roleTransition, sessionLogPath, runbookPhase) {
  ...
  const architectDiscoveryChat = role === "architect" && !selectedDirective && runbookPhase === "architect-discovery";
  ...
  if (role === "architect" && !selectedDirective) {
    nextActions.push(
      ...
      "After approval, transition to architect authoring runbook phase to create the directive session.",
    );
  }
  const doc = {
    ...
    operator_discovery: {
      required: architectDiscoveryChat ? false : !autoExecuteExecutorTask,
      checklist: [
        ...(architectDiscoveryChat
          ? [
            "Keep discovery conversational; no rigid checklist unless requested.",
            "Ask one clarifying question at a time and synthesize intent.",
            "Capture title, branch plan, and goals; wait for explicit approval before artifact creation.",
          ]
          : [
      ...
      before_any_command: architectDiscoveryChat ? false : !autoExecuteExecutorTask,
    },
    model_thinking_gate: {
      required: architectDiscoveryChat ? false : !autoExecuteExecutorTask,
      checklist: [
        ...(architectDiscoveryChat
          ? [
            "Skip model/thinking gates in discovery-only mode.",
          ]
          : [
