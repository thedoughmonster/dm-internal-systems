...
### 3) Production
- Deployed only from the `prod` branch
...
- Single repository
- Two long-lived branches: `dev` and `prod`
- Feature work happens on short-lived branches from `dev` using `feat/*`, `fix/*`, and `chore/*`
...
- Feature and maintenance PRs merge into `dev`
- Production release is a PR merge from `dev` into `prod`
...
- A PR is merged from `dev` into `prod`
...
**Local -> PR Preview -> Dev -> Prod = Production**
