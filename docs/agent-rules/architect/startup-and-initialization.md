...
20. When creating a new directive session, scaffold `<directive_slug>.meta.json` using `newdirective` (or `node ops_tooling/scripts/directives/create_directive_readme.mjs` fallback) before any manual intake edits.
