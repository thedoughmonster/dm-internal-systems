...
- Fallback command when `newdirective` is unavailable on `PATH`: `node ops_tooling/scripts/directives/create_directive_readme.mjs --title "<title>" --summary "<one line summary>"`.
...
- Fallback command when `newtask` is unavailable on `PATH`: `node ops_tooling/scripts/directives/create_directive_task.mjs --session "<session-dir-or-uuid>" --title "<title>" --summary "<one line summary>"`.
...
- Fallback command when `newhandoff` is unavailable on `PATH`: `node ops_tooling/scripts/directives/create_handoff.mjs ...`.
- Architect metadata updates must use `architect-updatemeta` (or `node ops_tooling/scripts/directives/update_directive_metadata.mjs --role architect ...` fallback).
...
- Local git hook installation command (one-time):
  - `bash ops_tooling/scripts/directives/install_git_hooks.sh`
